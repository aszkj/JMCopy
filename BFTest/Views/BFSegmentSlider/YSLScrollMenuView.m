...
#define kYSLScrollMenuViewWidth  Screen_width/4
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _itemSelectedTitleColor = BFColor(248, 212, 47, 1);
...
- (void)setItemTitleArray:(NSArray *)itemTitleArray
{
    if (_itemTitleArray != itemTitleArray) {
        ...
        for (int i = 0; i < itemTitleArray.count; i++) {
            CGRect frame = CGRectMake(0, 0, Screen_width/itemTitleArray.count, CGRectGetHeight(self.frame));
        ...
        _indicatorView.frame = CGRectMake(0, _scrollView.frame.size.height - kYSLIndicatorHeight, Screen_width/self.itemViewArray.count, kYSLIndicatorHeight);
...
- (void)setIndicatorViewFrameWithRatio:(CGFloat)ratio isNextItem:(BOOL)isNextItem toIndex:(NSInteger)toIndex
{
    ...
    if (isNextItem) {
        indicatorX = ((kYSLScrollMenuViewMargin + Screen_width/self.itemViewArray.count) * ratio ) + (toIndex * Screen_width/self.itemViewArray.count) + ((toIndex + 1) * kYSLScrollMenuViewMargin);
    } else {
        indicatorX =  ((kYSLScrollMenuViewMargin + Screen_width/self.itemViewArray.count) * (1 - ratio) ) + (toIndex * Screen_width/self.itemViewArray.count) + ((toIndex + 1) * kYSLScrollMenuViewMargin);
    }
    
    if (indicatorX < kYSLScrollMenuViewMargin || indicatorX > self.scrollView.contentSize.width - (kYSLScrollMenuViewMargin + Screen_width/self.itemViewArray.count)) {
        return;
    }
    
    _indicatorView.frame = CGRectMake(indicatorX, _scrollView.frame.size.height - kYSLIndicatorHeight, Screen_width/self.itemViewArray.count, kYSLIndicatorHeight);
...
- (void)layoutSubviews
{
    ...
    for (NSUInteger i = 0; i < self.itemViewArray.count; i++) {
        CGFloat width = Screen_width/self.itemViewArray.count;
