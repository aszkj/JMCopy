...
- (UIImage *)capture {
    ...
    */
//    UIGraphicsBeginImageContext(CGSizeMake(Screen_width, Screen_width));
    
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(Screen_width, Screen_width - 44), NO, 0.0);
    CGContextRef context = UIGraphicsGetCurrentContext();
    [self.superview.layer renderInContext:context];
    
    UIImage * img = UIGraphicsGetImageFromCurrentImageContext();
//    NSLog(@"%@",NSStringFromCGRect(convertRect));
//    UIImage * imgeee = [UIImage imageWithCGImage:CGImageCreateWithImageInRect(img.CGImage, CGRectMake(0, 0, Screen_width, Screen_width))];
    UIGraphicsEndImageContext();
    if (self.imageView.width < Screen_width || self.imageView.height < Screen_height) {
