...
- (void)handlePanGesture:(UIPanGestureRecognizer *)pan
{
    switch (pan.state) {
        case UIGestureRecognizerStateChanged: {
            ...
            _cellFakeView.center = CGPointMake(_cellFakeViewCenter.x + _panTranslation.x, _cellFakeViewCenter.y + _panTranslation.y);
            CGFloat center_x = _cellFakeView.center.x;
            CGFloat center_y = _cellFakeView.center.y;
            CGFloat collectionHeight = self.collectionView.frame.size.height;
            if (CGRectGetMaxY(_cellFakeView.frame) > Screen_width) {
                center_y = Screen_width - _cellFakeView.frame.size.height/2;
            }
            
            _cellFakeView.center = CGPointMake(center_x, center_y);
