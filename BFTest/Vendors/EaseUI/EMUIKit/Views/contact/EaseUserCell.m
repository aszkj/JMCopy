...
- (void)setModel:(BFSearchFrid *)model{
    _model = model;
    
    if ([_model.nikename length] > 0) {
        self.titleLabel.text = _model.nikename;
    }
    else{
        self.titleLabel.text = _model.jmid;
    }
    
    if ([_model.mybmp length] > 0){
        [self.avatarView.imageView sd_setImageWithURL:[NSURL URLWithString:_model.mybmp] placeholderImage:[UIImage imageNamed:@"appuserlogo.jpg"]];
    } else {
//        if (_model.avatarImage) {
            self.avatarView.image = [UIImage imageNamed:@"appuserlogo.jpg"];
//        }
    }
}
