...
{
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.avatarView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeTop multiplier:1.0 constant:EaseConversationCellPadding+5]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.avatarView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-EaseConversationCellPadding-5]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.avatarView attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeLeft multiplier:1.0 constant:EaseConversationCellPadding+5]];
...
- (void)setConmodel:(id<IConversationModel>)conmodel{
    _conmodel = conmodel;
...
/*
 ...
 ext = @{@"jmid":self.jmid,@"userIcon":self.avatarURLPath,@"nickename":self.nikename};

- (void)setModel:(BFConversationModel *)model{
    _model = model;
    if ([_model.title length] > 0) {
        self.titleLabel.text = _model.title;
    }
...
- (void)setModel:(BFConversationModel *)model{
    ...
    if ([_model.title length] > 0) {
        ...
        // 陌生人消息
        NSString *userid = model.conversation.conversationId;
        NSMutableArray *cacheschat = [BFChatHelper getDataArrayFromDB:@"ChatUserCaches"];
        if (cacheschat.count > 0) {
            for (NSDictionary *dicuser in cacheschat) {
                NSString *jmid = dicuser[@"jmid"];
                if ([userid isEqualToString:jmid]) {
                    self.titleLabel.text = dicuser[@"nikename"];
                    [self.avatarView.imageView yy_setImageWithURL:[NSURL URLWithString:dicuser[@"userIcon"]] placeholder:BFIcomImg];
                    self.iconurl = dicuser[@"userIcon"];
            ...
            NSDictionary *extdic = model.conversation.latestMessage.ext;
            if (extdic && extdic[@"nikename"]) {
                self.titleLabel.text = extdic[@"nikename"];
                [self.avatarView.imageView yy_setImageWithURL:[NSURL URLWithString:extdic[@"userIcon"]] placeholder:BFIcomImg];
                self.iconurl = extdic[@"userIcon"];
                if (_model.conversation.unreadMessagesCount == 0) {
                    _avatarView.showBadge = NO;
                    return;
                }else{
    ...
    if (self.showAvatar) {
        if ([_model.avatarURLPath length] > 0){
            [self.avatarView.imageView yy_setImageWithURL:[NSURL URLWithString:_model.avatarURLPath] placeholder:[UIImage imageNamed:@"jinmaiperson"]];
            
                   } else {
            if (_model.avatarImage) {
                self.avatarView.image = [UIImage imageNamed:@"jinmaiperson"];
            }
        }
        if (_model.conversation.unreadMessagesCount == 0) {
            _avatarView.showBadge = NO;
...
/*
    NSDictionary *dic = model.conversation.latestMessageFromOthers.ext;
    NSString *avater = dic[@"userIcon"];
    NSString *nickename = dic[@"nikename"];
    NSLog(@"%@ ----- %@",nickename,model.avatarURLPath);
    NSString *jmid = dic[@"jmid"];

    if ([nickename length] > 0) {
        self.titleLabel.text = nickename;
    }
    else{
        self.titleLabel.text = jmid;
    }
    if ([avater length] > 0){
        [self.avatarView.imageView sd_setImageWithURL:[NSURL URLWithString:avater] placeholderImage:BFIcomImg];
    } else {
        if (_model.avatarImage) {
            self.avatarView.image = BFIcomImg;
        }
    }
     */
    self.avatarView.size = CGSizeMake(self.avatarView.size.width, self.avatarView.size.width);
    self.avatarView.layer.cornerRadius = self.avatarView.size.width/2;
