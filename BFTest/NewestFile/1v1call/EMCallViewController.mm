...
- (void)_layoutSubviews
{
    ...
    self.remoteNameLabel.text = self.callSession.remoteUsername;
    
    BOOL isCaller = [self.callSession.sessionId isEqualToString:[BFUserLoginManager shardManager].jmId];
...
- (void)_setupRemoteVideoView
{
    if (self.callSession.type == EMCallTypeVideo && self.callSession.remoteVideoView == nil) {
        ...
        NSLog(@"self.callSession.remoteVideoView subViews ->%@",self.callSession.remoteVideoView.subviews);
...
- (IBAction)silenceAction:(id)sender
{
    ...
    if (self.silenceButton.selected) {
        [[BFHXDelegateManager shareDelegateManager]hangupCallWithReason:EMCallEndReasonNoResponse];
    } else {
//        [[BFHXDelegateManager shareDelegateManager] makeCallWithUsername:self.callSession.sessionId type:self.callSession.type];
    }
}
...
{
    [self _stopRing];
//    [[BFHXDelegateManager shareDelegateManager] answerCall:self.callSession.sessionId];
}
