...
- (NSString*)showName{
    if (self.type == EMConversationTypeChat) {
        return [BFHXManager getNameFromCurrentManagerListWithOtherJmid:self.conversationId callBack:^(NSString *name){
            ...
            didSelectConversationModel:(id<IConversationModel>)conversationModel
{
    if (conversationModel) {
        ...
        if (conversation) {
                BFChatViewController *chatController = nil;
            NSString *str = conversation.conversationId;
                chatController = [[BFChatViewController alloc] initWithConversationChatter:conversation.conversationId conversationType:conversation.type];

                chatController.title = conversationModel.title;
                chatController.hidesBottomBarWhenPushed = YES;
                [[BFHXDelegateManager shareDelegateManager].mainVC.navigationController pushViewController:chatController animated:YES];
        }
                                    ...
                                    modelForConversation:(EMConversation *)conversation
{
    ...
    if (model.conversation.type == EMConversationTypeChat) {
        BFUserInfoModel *userModel = [BFHXManager getUserInfoWithOtherJmid:conversation.conversationId];

        if (model && userModel.jmid != nil) {
                model.title= userModel.name;
      ...
      latestMessageTitleForConversationModel:(id<IConversationModel>)conversationModel
{
    ...
    if (lastMessage) {
        ...
        if (lastMessage.direction == EMMessageDirectionReceive) {
            NSString *from = [BFHXManager getNameFromCurrentManagerListWithOtherJmid:lastMessage.from callBack:^(NSString *name) {
...
- (void)setupSearchController
{
    ...
    [self.resultController setDidSelectRowAtIndexPathCompletion:^(UITableView *tableView, NSIndexPath *indexPath) {
        ...
        BFChatViewController *chatController;
