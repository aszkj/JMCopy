...
- (CAShapeLayer *)progressLayer{
    if (!_progressLayer) {
        ...
        _progressLayer.strokeColor = BFThemeColor.CGColor;
        _progressLayer.lineWidth = DefaultLineWidth;
    }
...
-(void)setProgress:(CGFloat)Percentage Animated:(BOOL)animated
{
    ...
    if (animated) {
        ...
        if (self.style ==  ZFProgressViewStyleNone || self.style == ZFProgressViewStyleImageSegment) {
            ...
            self.progressLayer.strokeEnd = _Percentage;
        }
        else
        {
...
- (void)stopanimate{
    CFTimeInterval time = [_progressLayer convertTime:CACurrentMediaTime() fromLayer:nil];
    _progressLayer.speed = 0.0;
    _progressLayer.timeOffset = time;
}
